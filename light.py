...
from .IdealLed import COMMAND_LIGHT_ON_HEX, COMMAND_LIGHT_OFF_HEX
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        async with self.coordinator.async_connect_and_update() as device:
            if ATTR_BRIGHTNESS in kwargs:
                ...
                if not self.is_on:
                    await device.send_command(COMMAND_LIGHT_ON_HEX)

    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn the entity off."""
        if self.is_on:
            async with self.coordinator.async_connect_and_update() as device:
                await device.send_command(COMMAND_LIGHT_OFF_HEX)
